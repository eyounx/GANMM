...
def mnist_generator(data, batch_size, n_labelled, limit=None):
    ...
    def get_epoch():
        ...
        image_batches = images.reshape(-1, batch_size, 19)
...
def load(batch_size, test_batch_size, scale=1.0, n_labelled=None):
    ...
    with open('Data/segmentation.pkl', 'rb') as f:
        cont = pickle.load(f)
        full_ft,full_lbl = cont[0], cont[1]
        full_ft = full_ft[:2300,:]
        full_lbl = full_lbl[:2300]
...
def splitGenerator(batch_size, scale=1.0):
    ...
    def getEpoch(num):
        length = int(data_size/(batch_size*7))
        i=num*length
        while True:
            ...
            if i==(num+1)*length:
                i=num*length
    return [getEpoch(0),getEpoch(1),getEpoch(2),getEpoch(3),getEpoch(4),getEpoch(5),getEpoch(6)]
